	...
	<div class="row eqqualheight">
		<aside class="col-sm-2 sidebar">
			...
			<ul class="group-list list-unstyled">
				<li class="active"><a href="javascript:void(0)">View group</a></li>
		...
		<div class="col-sm-8" >
			...
			<div class="page-header" style="margin-top:20px;">
				<a href="#" class="btn btn-default btn-sm noround" data-toggle="modal" data-target="#addmembers">Add Members</a>
				<div class="btn-group">
					 <button type="button" class="btn btn-default dropdown-toggle btn-sm noround" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
			  ...
			  <div class="modal-dialog" role="document">
				<div class="modal-content">
				  ...
				  <div class="modal-body">
					<form method="post" id="addmember">
						<div class="form-group">
							<input type="search" name="search-member" id="searchmember" class="form-control" placeholder="Search by Name"/>
				  ...
				  <div class="modal-footer">
					<button type="button" class="btn btn-default noround" data-dismiss="modal">Close</button>
					<button type="button" class="btn btn-primary noround" id="add-member">Add Members</button>
				  </div>
				</div>
			  </div>
					...
					jQuery(function($){
						$('#searchmember').on('keyup', function(){
							if($(this).val().length >= 3){
								$(this).next().fadeIn(); //remove loader with ajax success
								//Ajax Call
								$.ajax({
									url: "<?php echo base_url();?>backend/sitegroup/members",
									data: {
										'groupid':'<?php echo $group[0]->id ?>',
										'name': $(this).val(),
									},
									type: "POST",
									dataType: 'json',
									...
									complete : function(response) {
										$('#result-list tbody').html(response.responseJSON);
						...
						$('#add-member').on('click', function(){
							var selectedMembers = [];
							$('#result-list input[name="add_member"]:checked').each(function() {
								selectedMembers.push($(this).val());
							});	
							$.ajax({
								url: "<?php echo base_url();?>backend/sitegroup/addmembers",
								data: {
									'groupid':'<?php echo $group[0]->id ?>',
				...
				jQuery(function($){
						$('#addpostform input[name="attachment"]').on('change', function(){
							...
							$("#fileUploadForm").on('submit', function(e) {
									...
									$.ajax({
										type:'POST',
										url: "<?php echo base_url(); ?>"+"backend/sitegroup/ajaxSubmitPost",
										cache: false,
										contentType: false,
										processData: false,
										data: form_data,
						...
						$('#addPost').click(function(){
								 ...
								 if((postDesc.length === 0) && (postVal.length === 0)){
									 ...
									 $.ajax({
										 type:'POST',
										 url: "<?php echo base_url(); ?>"+"backend/sitegroup/ajaxAddPost",
										 dataType: 'json',
										 data: {
											 'postDesc' : postDesc,
			...
			<section id="postview">
				...
				<?php foreach ($group_posts as $key => $val){ ?>
			...
			<script>
				jQuery(function($){
					$('.lickCommentShare>li.comment>a').click(function(){
						...
						$.ajax({
							url: "<?php echo base_url();?>backend/sitegroup/vieaAllComments",
							data: {'postid':postid},
							type: "POST",
							dataType: 'json',
							context: this,
			...
			<nav aria-label="...">
			  <ul class="pager">
				<?php if($page_num != 1){ ?>
				<li class="previous"><a href="<?php echo base_url(); ?>backend/sitegroup/view/<?php echo $group[0]->slug.'/'.($page_num-1); ?>"><span aria-hidden="true">&larr;</span> Newer </a></li>
				<?php } ?> 
				<li class="next"><a href="<?php echo base_url(); ?>backend/sitegroup/view/<?php echo $group[0]->slug.'/'.($page_num+1); ?>">Older  <span aria-hidden="true">&rarr;</span></a></li>
			  </ul>
			</nav>
			<!--End of pagination-->
		...
		<div class="right-sidebar">
			<?php if(count($joined) > 0) {	?>
			<button type="button" class="btn btn-primary noround" id="leave-group" data-groupid="<?php echo $group[0]->id;?>">Leave Group <i class="far fa-check-square"></i></button>
			<?php } ?>
			<?php if(count($joined) <= 0) {	?>
			<button type="button" class="btn btn-primary noround" id="join-group" data-groupid="<?php echo $group[0]->id;?>">Join Group <i class="far fa-check-square"></i></button>
			<?php } ?>
		</div>
		</div>
		...
		<script>
		 $(document).ready(function(){
			...
			$('.like-post').click(function(){
				var postid = $(this).data('postid');
				$.ajax({
				  url: "<?php echo base_url();?>backend/sitegroup/postlike",
					data: {
						'postid':postid,
			...
			$('form.comment-form').submit(function(e){
				...
				$.ajax({
				  url: "<?php echo base_url();?>backend/sitegroup/addComment",
					data: {
						'postid':postid,
						'group_id': $('input[name=groupId]').val(),
						'comment': comment,
