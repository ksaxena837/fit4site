...
class SqlQueries{
	...
	*/
		...
		$sqlqueries['admin_get_active_guests_count'] = "select SUM(guest) as totalactiveguest FROM (select count(userid) as guest from cometchat_status where userid >{firstguestid} and lastactivity >= '{sent}' UNION select count(userid) as guest from cometchat_status_archive where userid >{firstguestid} and lastactivity >= '{sent}') as totalactiveguest";
...
$sqlqueries['weekly_privateMessages'] = "SELECT COUNT(id) AS message, DAYNAME(FROM_UNIXTIME(sent)) AS day from (SELECT id, cometchat.sent FROM cometchat UNION SELECT id, cometchat_archive.sent FROM cometchat_archive ) as temp WHERE DATE(FROM_UNIXTIME(sent)) >= DATE(NOW()) - INTERVAL 7 DAY GROUP BY DAY";
...
$sqlqueries['today_active_users_count'] = "SELECT SUM(users) AS totalactiveusers, SUM(guest) AS totalactiveguest FROM (SELECT 0 AS users,COUNT(userid) AS guest FROM cometchat_status WHERE userid > {firstguestid} AND DATE( FROM_UNIXTIME(cometchat_status.lastactivity)) = CURDATE() UNION SELECT 0 AS users, COUNT(userid) AS guest FROM cometchat_status_archive WHERE userid > {firstguestid} AND DATE( FROM_UNIXTIME( cometchat_status_archive.lastactivity ) ) = CURDATE() UNION SELECT COUNT(userid) AS users, 0 AS guest FROM cometchat_status WHERE userid < {firstguestid} AND DATE( FROM_UNIXTIME(cometchat_status.lastactivity) ) = CURDATE() UNION SELECT COUNT(userid) AS users,0 AS guest FROM cometchat_status_archive WHERE userid < {firstguestid} AND DATE( FROM_UNIXTIME( cometchat_status_archive.lastactivity)) = CURDATE()) AS x";

$sqlqueries['weekly_active_users'] = "SELECT COUNT(userid) as usercount, IF(userid > {firstguestid}, 'guest','user') as usertype, date as day from (SELECT * FROM ( SELECT userid AS userid, DAYNAME(date) AS date FROM ( SELECT cometchat_chatroommessages.userid AS userid, DATE(FROM_UNIXTIME(cometchat_chatroommessages.sent)) as date FROM cometchat_chatroommessages GROUP BY userid, date UNION SELECT cometchat_chatroommessages_archive.userid AS userid, DATE(FROM_UNIXTIME(cometchat_chatroommessages_archive.sent)) as date FROM cometchat_chatroommessages_archive GROUP BY userid, date ) AS temp WHERE date >= DATE(NOW()) - INTERVAL 7 DAY) AS groups UNION SELECT * FROM (SELECT userid, DAYNAME(date) AS date FROM ( SELECT cometchat.from AS userid, DATE(FROM_UNIXTIME(cometchat.sent)) as date FROM cometchat GROUP BY userid, date UNION SELECT cometchat_archive.from AS userid, DATE(FROM_UNIXTIME(cometchat_archive.sent)) as date FROM cometchat_archive GROUP BY userid, date ) AS temp WHERE date >= DATE(NOW()) - INTERVAL 7 DAY) AS oneoneone) as activeusers group by date, usertype";

$sqlqueries['monthly_active_users'] = "SELECT COUNT(userid) AS usercount, IF(userid > {firstguestid}, 'guest', 'user') AS usertype, Week FROM ( SELECT * FROM ( SELECT userid AS userid, WEEK(DATE) AS Week FROM ( SELECT cometchat_chatroommessages.userid AS userid, DATE( FROM_UNIXTIME( cometchat_chatroommessages.sent ) ) AS DATE FROM cometchat_chatroommessages GROUP BY userid, DATE UNION SELECT cometchat_chatroommessages_archive.userid AS userid, DATE( FROM_UNIXTIME( cometchat_chatroommessages_archive.sent ) ) AS DATE FROM cometchat_chatroommessages_archive GROUP BY userid, DATE ) AS temp WHERE MONTH(DATE) = MONTH(CURDATE())) AS groups UNION SELECT * FROM ( SELECT userid, WEEK(DATE) AS Week FROM ( SELECT cometchat.from AS userid, DATE(FROM_UNIXTIME(cometchat.sent)) AS DATE FROM cometchat GROUP BY userid, DATE UNION SELECT cometchat_archive.from AS userid, DATE( FROM_UNIXTIME(cometchat_archive.sent) ) AS DATE FROM cometchat_archive GROUP BY userid, DATE ) AS temp WHERE MONTH(DATE) = MONTH(CURDATE())) AS oneoneone ) AS activeusers GROUP BY Week, usertype";

$sqlqueries['all_active_users'] = "SELECT COUNT(userid) AS usercount, IF(userid > {firstguestid}, 'guest', 'user') AS usertype, SUBSTR(MONTHNAME, 1, 3) as month, SUBSTR(YEAR, 3, 2) as year FROM ( SELECT * FROM ( SELECT userid AS userid, MONTHNAME, YEAR FROM ( SELECT cometchat_chatroommessages.userid AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR,  MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME FROM cometchat_chatroommessages GROUP BY userid, MONTHNAME UNION SELECT cometchat_chatroommessages_archive.userid AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR,  MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME FROM cometchat_chatroommessages_archive GROUP BY userid, MONTHNAME ) AS temp) AS groups UNION SELECT * FROM ( SELECT userid, MONTHNAME AS MONTHNAME, YEAR FROM ( SELECT cometchat.from AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR,  MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME FROM cometchat GROUP BY userid, MONTHNAME UNION SELECT cometchat_archive.from AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR,  MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME FROM cometchat_archive GROUP BY userid, MONTHNAME ) AS temp  ) AS oneoneone ) AS activeusers GROUP BY MONTHNAME, YEAR, usertype ORDER BY year";
